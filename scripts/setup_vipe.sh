...
if ! conda env list | awk '{print $1}' | grep -Fxq "${VIPE_ENV_NAME}"; then
  conda env create --name "${VIPE_ENV_NAME}" --file "${vipe_dir}/envs/base.yml"
fi

conda_run "${VIPE_ENV_NAME}" python -m pip install \
  --requirement "${vipe_dir}/envs/requirements.txt" \
  --extra-index-url https://download.pytorch.org/whl/cu128
conda_run "${VIPE_ENV_NAME}" python -m pip install --no-build-isolation "${vipe_dir}"
conda_run "${VIPE_ENV_NAME}" python -c "import vipe; print('ViPE import: OK')"
